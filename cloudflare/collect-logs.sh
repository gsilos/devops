#!/usr/bin/env bash
echo "please remember to fill your cloudflare dotfile "
source ~/.cloudflare
...
RELATIVE_TIME_START=0		# in hours
LOGEXTENSION=0
...
TMPLOG=./data/cloudflare.log.${LOGEXTENSION}.json
...
if [ $os = "Darwin" ]; then
	...
	qst=`date --utc -d "${QUERY_TIME_START} minutes ago"`
	start=`date --utc -d "${qst}-${RELATIVE_TIME_START} minutes" +'%Y-%m-%dT%H:%M:%SZ'`
	qet=`date --utc -d "${QUERY_TIME_END} minutes ago"`
	end=`date --utc -d "${qet}-${RELATIVE_TIME_START} minutes" +'%Y-%m-%dT%H:%M:%SZ'`
else
